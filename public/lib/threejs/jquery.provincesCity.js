...
$.fn.ProvinceCity = function(){
	...
	//插入3个空的下拉框
	//_self.append("提取事由：<select></select>");
	//_self.append("提取范围：<select></select>");
	//_self.append("<select></select>");
	//分别获取3个下拉框
	//var $sel1 = _self.find("select").eq(0);
	//var $sel2 = _self.find("select").eq(1);
	//var $sel3 = _self.find(".intro").eq(2);
	//分别获取3个下拉框
	var $sel1 = _self.find("#condition");
	var $sel2 = _self.find("#range");
	var $sel3 = _self.find("#intro");
	//默认省级下拉
	...
	//默认的2级城市下拉
	if(_self.data("city2")){
		$sel3.append(_self.data("city2")[0]);
	}
	//省级联动 控制
	var index1 = "" ;
